...
public class _VertexHeap {
		
	private int capicity;
	private _WeightedEdge[] pvertex;
	private int count = 0;
	private int next = 1;
	...
	public _VertexHeap(int size) {
		this.capicity = size+1;
		pvertex = new _WeightedEdge[size+1];
	}
	...
	public boolean insert(_WeightedEdge pv) {
		if (isFull())
			return false;
		int i = next;
		pvertex[i] = pv;
	...
	public _WeightedEdge remove() {
		_WeightedEdge result;
		if (count == 0)
			return null;
		result = pvertex[1];
	...
	public boolean contains(_WeightedEdge we) {
		for (int i=1; i <= count; i++) {
